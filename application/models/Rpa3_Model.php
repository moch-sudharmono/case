...
class Rpa3_Model extends CI_Model 
{
	protected 	 $table = 'rpa3';	 	 
	protected 	 $primary_key = 'id_rpa3';
	...
	{
		$this->db->order_by($this->table .".tgl_rpa3" . " desc ");
		$this->db->join('rpa2','rpa2.id_rpa2='.$this->table .'.rpa2_id');
	...
	public function displayBetweenData($start, $finish)
	{
		...
		//$this->db->join('jpu_rpa1','jpu_rpa1.rpa1_id=rpa1.id_rpa1');
		//$this->db->join('attorney','jpu_rpa1.attorney_id=attorney.id_attorney');
		$this->db->where("tgl_rpa3 >= '".$start."'");
		$this->db->where("tgl_rpa3 <= '".$finish."'");
		$query = $this->db->get($this->table);
		return $query->result_array();
	}
