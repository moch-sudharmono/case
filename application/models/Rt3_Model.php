...
class Rt3_Model extends CI_Model 
{
	...
	{
		$this->db->join('rp12','rp12.id_rp12 ='.$this->table.'.rp12_id');
		$this->db->join('rp9','rp9.id_rp9 =rp12.rp9_id');
	...
	public function displayBetweenData($start, $finish)
	{
		...
		$this->db->where("tgl_rt3 >= '".$start."'");
		$this->db->where("tgl_rt3 <= '".$finish."'");
		$query = $this->db->get($this->table);
		return $query->result_array();
	}
